...
function renderSubcategories() {
    ...
    if (subcategoriesForCurrentCategory.length > 0) {
        ...
        allSubBtn.dataset.subcategoryId = `All ${categoryName}`; // Use a unique identifier for 'All'
        ...
        subcategoriesForCurrentCategory.forEach(sub => {
            ...
            subBtn.dataset.subcategoryId = sub.subcatid;
...
function renderMenuItems() {
    ...
    if (currentSubcategory && currentSubcategory.startsWith('All ') === false) { // Check if it's not an "All Category" subcategory
        // Find the actual subcategory ID from the name
        const subcatObj = menuSubcategories.find(sub => sub.subcategory === currentSubcategory && sub.categoryid === currentCategory);
        if (subcatObj) {
            filteredItems = filteredItems.filter(item => item.subcategoryid === subcatObj.subcatid);
        } else {
            filteredItems = []; // No items if subcategory not found
        }
    }
