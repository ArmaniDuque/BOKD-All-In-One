...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Montserrat', sans-serif; }
        ...
        #start-btn {
            padding: 22px 60px; font-size: 1.2rem; font-weight: 800;
            letter-spacing: 3px; color: white; background: linear-gradient(45deg, #c41e3a, #ff4d4d);
            border: none; cursor: pointer; border-radius: 50px; transition: 0.3s;
        }

        /* Background */
        .background-container { position: fixed; top: -5%; left: -5%; width: 110%; height: 110%; z-index: -2; }
        .bg-media { width: 100%; height: 100%; object-fit: cover; filter: brightness(35%) contrast(110%); }
        ...
        .company-logo {
            width: 800px; max-width: 90vw; margin-bottom: -40px;
            transition: all 1s ease;
        }

        /* SCHEDULE BOX (La Dulce Navidad) */
        #schedule-box {
            position: absolute;
        ...
        #schedule-box.show { opacity: 1; transform: translateY(0); }

        .sched-title { font-size: 2.5rem; color: #ff69b4; font-style: italic; margin-bottom: 10px; }
        .sched-date { font-size: 1.2rem; letter-spacing: 2px; margin-bottom: 20px; border-bottom: 1px solid #ffd700; padding-bottom: 10px; }
        .sched-item { font-size: 1.3rem; margin: 10px 0; display: flex; justify-content: space-between; text-align: left; }
        .sched-time { color: #ffd700; font-weight: bold; margin-right: 20px; min-width: 150px; }

        /* THANK YOU CONTAINER */
        .thank-you-container { margin-bottom: 3px; transition: all 0.5s ease; }
        #fixed-thanks { font-size: 2.2rem; font-weight: 400; color: #ffffff; letter-spacing: 2px; margin-bottom: 2px; }
        #family-names { font-size: 6.5rem; font-weight: 800; color: #ffd700; text-shadow: 0 0 20px rgba(255, 215, 0, 0.6);margin-bottom: 2px; }

        /* TIMER */
        #timer { display: flex; gap: 40px; }
        .time-block span { display: block; font-size: 10rem; font-weight: 900; }
        .time-block p { font-size: 2.2rem; text-transform: uppercase; letter-spacing: 5px; color: #ffd700; }

        /* RAFFLE UI */
        #raffle-btn {
            position: fixed; bottom: 30px; right: 30px; z-index: 100;
            padding: 15px 30px; background: #ffd700; color: #000;
            font-weight: 900; border: none; border-radius: 10px; cursor: pointer;
            box-shadow: 0 0 20px rgba(255, 215, 0, 0.5); transition: 0.3s;
            letter-spacing: 1px;
        }
        #raffle-btn:hover { transform: scale(1.1); background: #fff; }
        ...
        .winner-display { font-size: 6rem; font-weight: 900; color: #ffd700; text-shadow: 0 0 30px #ffd700; margin: 40px 0; }

        /* LAST MINUTE (60s) */
        .last-minute .time-block:not(.seconds-block) { display: none; }
        .last-minute .thank-you-container, .last-minute #schedule-box, .last-minute #raffle-btn { display: none !important; opacity: 0 !important; } 
        .last-minute .company-logo { position: absolute; width: 100vw; opacity: 0.15; z-index: -1; animation: logo-flash 1s infinite alternate; }
        .last-minute .seconds-block span { font-size: 35vw; animation: pulse-glow 1s infinite alternate; }

        /* CELEBRATION */
        #celebration-text { font-size: 10rem; font-weight: 900; background: linear-gradient(to bottom, #fff, #ffd700); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 20px; }
        .celebrating #family-names { font-size: 10rem; line-height: 1; animation: text-flash 1.0s infinite alternate; }
        
        @keyframes logo-flash { 0% { opacity: 0.1; } 100% { opacity: 0.25; } }
        @keyframes text-flash { 0% { text-shadow: 0 0 20px #ffd700; opacity: 0.7; } 100% { text-shadow: 0 0 60px #ffd700, 0 0 100px #ffffff; opacity: 1; } }
...
<body>
    ...
    <div id="raffle-modal">
        ...
        <button onclick="closeRaffle()" style="background: none; border: 1px solid white; color: white; padding: 10px 30px; cursor: pointer; border-radius: 5px;">CLOSE</button>
    </div>

    <audio id="bg-music" loop><source src="christmas.mp3" type="audio/mpeg"></audio>
    <audio id="fireworks-sound"><source src="Fireworks.mp3" type="audio/mpeg"></audio>
    ...
    <div class="content" id="main-content">
        ...
        <div id="schedule-box">
            ...
            <div class="sched-date">DECEMBER 24, 2025 | 7PM - 12MN</div>
        ...
        <div id="countdown-wrap">
            <div id="timer">
                ...
                <div class="time-block seconds-block"><span id="seconds">00</span><p>Secs</p></div>
            </div>
        </div>
    ...
    <script>
        ...
        const families = [ 
            ...
            "Wong Family", "Krakowska Family", "Ignacio Family", "Kim Family", "Seo Family", "Gilbert Family", "Huang Family",
            ...
            "He Family", "Chen Family", "Chan Family", "Lee Family", "Burns Family", "Mr. Park Family", "Priclla Family", " Michael Filp, Magdalena, David ",
        ];
        ...
        function openRaffle() {
            ...
            const shuffleInterval = setInterval(() => {
                winnerName.innerText = families[Math.floor(Math.random() * families.length)];
                shuffleCount++;
                if (shuffleCount > 30) {
                    clearInterval(shuffleInterval);
                    const finalWinner = families[Math.floor(Math.random() * families.length)];
                    winnerName.innerText = finalWinner.toUpperCase();
                    confetti({ particleCount: 200, spread: 90, origin: { y: 0.6 }, colors: ['#ffd700', '#ffffff'] });
                }
            }, 100);
        }

        function closeRaffle() { raffleModal.style.display = 'none'; }
        ...
        function triggerCelebration() {
            ...
            const randomFireworks = setInterval(function() {
                ...
                confetti({
                    particleCount: 80, startVelocity: 30, spread: 360,
        ...
        class Particle {
            ...
            reset() { this.x = Math.random() * canvas.width; this.y = Math.random() * canvas.height; this.size = Math.random() * 2 + 1; this.speed = Math.random() * 0.5 + 0.2; this.wind = Math.random() * 0.2 - 0.1; this.opacity = Math.random() * 0.5 + 0.3; }
