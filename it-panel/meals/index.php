...
<body>
    ...
    <button id="raffle-trigger">DRAW RAFFLE 🎁</button>

    <div id="raffle-modal">
        <h2 style="font-size: 2rem; letter-spacing: 5px;">THE WINNER IS...</h2>
        <div id="winner-display" class="winner-text">Spinning...</div>
        <button onclick="closeRaffle()"
            style="margin-top: 40px; background: none; border: 1px solid white; color: white; padding: 10px 20px; cursor: pointer;">CLOSE</button>
    </div>
    ...
    <div class="content" id="main-content">
        ...
        <div class="thank-you-container" id="names-box">
            ...
            <div id="family-names">Duque Family</div>
        </div>
    ...
    // Raffle Logic
    document.getElementById('raffle-trigger').addEventListener('click', () => {
        raffleModal.style.display = 'flex';
        winnerDisplay.innerText = "Spinning...";

        setTimeout(() => {
            const families = ["Duque Family", "Ong Family", "Palima Family"];
            const winner = families[Math.floor(Math.random() * families.length)];
            winnerDisplay.innerText = winner.toUpperCase();

            // Blast confetti for winner
            confetti({
                particleCount: 150,
                spread: 70,
                origin: {
    ...
    const targetDate = new Date("Dec 24, 2025 23:59:59").getTime();
