                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-2 mb-3 ">
                            <div class="side-buttons">
                                ...
                                <p id="selectednoofroomsDisplay" class="text-sm font-semibold text-blue-700 mb-4">
                                    No of Rooms: <span class="text-gray-500">None</span>
                                </p>
                        ...
                        c.firstname, c.lastname, rt.code
                    ...
                    WHERE 1=1"; // Start with a true condition to easily append AND clauses
                            ...
                            <div class="overflow-hidden">
                                ...
                                <table id="hmsReservationsTable" class="display min-w-full bg-white">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                        ?>
                                        <tr data-reservation-id="<?php echo htmlspecialchars($row['reservationid']); ?>"
                                            data-noofrooms-id="<?php echo htmlspecialchars($row['noofrooms']); ?>">
                                            ...
                                            <td><?php echo htmlspecialchars($row['blockecodeid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['companyid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['groupid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['currencyid'] ?? 'N/A'); ?></td>
                                            ...
                                            <td><?php echo htmlspecialchars($row['marketid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['sourceid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['agentid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['originid'] ?? 'N/A'); ?></td>
                                            <td><?php echo htmlspecialchars($row['paymenttypeid'] ?? 'N/A'); ?></td>
...
$(document).ready(function() {
    ...
    function updateButtonStates() {
        ...
        if (selectedReservationId) {
            ...
            $('#selectednoofroomsDisplay span').text(selectednoofroomsId);
        } else {
            $('#selectedReservationIdDisplay span').text('None');
            $('#selectednoofroomsDisplay span').text('None');
        }
    }
    ...
    $('#hmsReservationsTable tbody').on('click', 'tr', function() {
        ...
        if ($(this).hasClass('selected-row')) {
            ...
            selectednoofroomsId = $(this).data('noofrooms-id');
        }
    ...
    $('#splitBtn').on('click', function() {
        if (selectedReservationId) {
            window.location.href = 'split.php?reservationid=' + selectedReservationId '?noofrooms=' +
                selectednoofroomsId;
        }
    });
