...
<div id="upload">
    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count > 1) {
                $x = $emapData[0];
                $alumni = mysqli_query($conn, "select * from server WHERE serverid='$x'") or die(mysqli_error());
                $alum = mysqli_fetch_array($alumni);
                $count1 = mysqli_num_rows($alumni);
                if ($count1 == '0') {
                    mysqli_query($conn, "INSERT INTO server VALUES('','$emapData[0]','$emapData[1]','$emapData[2]','$emapData[3]','$emapData[4]','$emapData[5]','$emapData[6]')");
